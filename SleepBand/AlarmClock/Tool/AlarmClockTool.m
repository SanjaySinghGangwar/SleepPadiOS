...
+ (AlarmClockTool *)sharedAlarmClockTool
{
    static AlarmClockTool *__sharedAlarmClockTool = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __sharedAlarmClockTool = [[AlarmClockTool alloc] init];
        
    });
    
    return __sharedAlarmClockTool;
}

-(void)setClosePlay:(BOOL)closePlay{
    if (closePlay && _player) {
        [_player stop];
    }
}
...
- (void)GetAlarmClockModel{
    ...
    if ([AlarmClockModel GetAllAlarmClockEvent].count == 0) {
        ...
        for (AlarmClockModel * model in _alarmClockArray) {
            ...
            NSLog(@"timeCount = %d",(int)timeCount);
            if (timeCount <= 0 && timeCount > -59 && model.clockId == 99999) {
                [[UIApplication sharedApplication] beginReceivingRemoteControlEvents];
                self.alert=[[UIAlertView alloc]initWithTitle:NSLocalizedString(@"ACMVC_Tips", nil) message:model.clockDescribe delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", nil) otherButtonTitles:nil];
                self.alert.delegate=self;
                [self.alert show];
                NSString * path=[[NSBundle mainBundle]pathForResource:model.clockMusic ofType:nil];
                NSURL * url=[NSURL fileURLWithPath:path];
                NSError * error;
                _player=[[AVAudioPlayer alloc]initWithContentsOfURL:url error:&error];
                _player.numberOfLoops=14;    //无限循环  =0 一遍   =1 两遍    =2 三遍     =负数  单曲循环
                _player.volume=2;          //音量
            ...
            // >0表示定时任务没有被执行
            //if (timeCount > 0 && timeCount < 86400) {
            if (timeCount > 0 && timeCount < 86400) {
                //当前闹钟，取时间最近的闹钟
                if (timeCount < midTimeCoun || midTimeCoun == 0) {
                    self.timerWillPlay          = model;
                    self.count                  = timeCount;
                    midTimeCoun                 = timeCount;
                }
...
//        [session setCategory:AVAudioSessionCategoryPlayback error:nil];
        //让app支持接受远程控制事件
        [[UIApplication sharedApplication] beginReceivingRemoteControlEvents];
        //提示框弹出的同时，开始响闹钟 RTVC_HeartRateTitle RTVC_RespiratoryRateTitle
//        self.alert=[[UIAlertView alloc]initWithTitle:NSLocalizedString(@"ACMVC_Tips", nil) message:NSLocalizedString(@"ACMVC_Close", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", nil) otherButtonTitles:nil];
        self.alert=[[UIAlertView alloc]initWithTitle:NSLocalizedString(@"ACMVC_Tips", nil) message:_timerWillPlay.clockDescribe delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", nil) otherButtonTitles:nil];
        self.alert.delegate=self;
        [self.alert show];
